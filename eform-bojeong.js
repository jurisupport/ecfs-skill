// 보정서(기본형) 템플릿 — 이폼(전자문서작성)으로 항목별 입력 후 작성완료(최종문서확인)까지
// 파일첨부방식 아님. 전자서명/제출 안 함.
...
const NOMINEE = '○○○';                 // ← 서류명의인(당사자명, 자동등록 실패 시 사용)
const ATTACH_NAME = '납부확인서';
const SHOT = '/tmp/ecfs-cost/eform';
...
(async () => {
  ...
  try {
    ...
    // ── 4-1. 보정서명령 목록: 첫 번째(인지대,송달료 보정명령) 선택 → 등록
    await page.evaluate(() => {
